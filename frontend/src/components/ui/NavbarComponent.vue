<template>
  <v-layout class="mb-10 pb-10">
      ...
      <v-btn :active="false" :ripple="false" size="x-large" to="/">
        <v-app-bar-title class="mx-n3 text-color">
          <div class="d-flex align-center">
            <v-img class="me-1" :src="require('@/../public/img/icons/mstile-150x150.png')" style="width: 32px">
            </v-img>
      ...
      <template v-slot:append>
        <section class="d-flex align-center justify-center">
          <router-link to="/profile">
            <img alt="Avatar" class="rounded-circle ms-2 mt-2" :src="userFromDB?.profileImg" style="width: 40px" />
          </router-link>
          <v-app-bar-nav-icon @click.stop="drawer = !drawer" color="white" :ripple="false" size="x-large"
            variant="plain" />
        </section>
      </template>
    </v-app-bar>
    ...
    <!--? SIDEBAR -->
    <v-navigation-drawer v-model="drawer" app class="bg-primary-medium text-color" location="end" temporary>
      <v-list-item :active="false" class="sidebar-header-bg-color" to="/">
        <v-list-item-title class="text-h6 py-3">
          <div class="d-flex align-center">
            <v-icon class="me-2" :color="navLinks[0].color" :icon="navLinks[0].icon" size="large" />
            {{ navLinks[0].text }}
          </div>
        </v-list-item-title>
      </v-list-item>
      <div class="sidebar-item-bg-color">
        <v-list color="blue" dense nav>
          <v-list-item v-for="(link, index) in filteredLinks" :key="index" :color="link.color" class="sidebar-bg-color"
            :to="link.url">
            <v-list-item-title>
              <div class="d-flex align-center">
                <v-icon class="me-2" :color="link.color" :icon="link.icon" size="x-large" />
                {{ link.text }}
              </div>
            </v-list-item-title>
          </v-list-item>
