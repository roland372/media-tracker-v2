<template>
  <CardComponent :title="`${mediaType} Stats`">
    <section v-for="(item, index) in progress" :key="index" class="text-color">
      <v-progress-linear :class="{ 'text-black': item.color === 'white' }" :color="item.color" height="20"
        :model-value="item.value"><strong>{{ Number.isNaN(item.value) ? 0 : Math.ceil(item.value)
    ...
    <section class="d-sm-flex justify-space-between text-color">
      <section class="text-start mb-3 mb-xs-0">
        <div v-for="(item, index) in status" :key="index" class="d-flex align-center justify-space-between">
      ...
      <section class="d-flex flex-column">
        <div v-for="(item, index) in stats" :key="index" class="d-flex justify-space-between">
          <div>
            <b>{{ item.name }}</b>
          </div>
