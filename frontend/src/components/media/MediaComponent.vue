<template>
  <CardComponent :title="title">
    ...
    <section v-else class="grid-container">
      <section v-for="(image, index) in media.slice(
        0,
        allMedia ? displayMediaFlag : props.media.length
      )" :key="index" class="d-flex align-center justify-center">
        <MediaCard class="media-img-card" :media="image" :media-type="mediaType" />
      </section>
    </section>
    <ButtonText v-if="allMedia && media.length > 20" @click="displayMedia" :append-icon="displayMediaFlag === 20 ? 'mdi-arrow-down-bold' : 'mdi-arrow-up-bold'
      " class="mt-3" color="indigo" :text="displayMediaFlag === 20 ? 'Display All' : 'Display Less'" />
  </CardComponent>
</template>
